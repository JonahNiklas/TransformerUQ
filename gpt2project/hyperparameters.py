...
class GPT2ModelConfig(BaseModel):
    ...
    dropout: float = Field(default=0.073, description="Dropout rate")
    dropout_mlp_input: float = Field(
        default=0.073 / 2, description="Dropout rate for MLP input"
    )
    dropout_pre_embedding: float = Field(
        default=0.073 / 2, description="Dropout rate for pre-embedding"
    )
