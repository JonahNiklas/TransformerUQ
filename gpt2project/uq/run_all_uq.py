...
from gpt2project.uq.gpt_aq_funcs import BALD, AcquisitionFunctionGPT, BeamScore, BLEUVar, mpnet_cosine
...
def main() -> None:
    ...
    tasks: List[
        ...
        ("LAMBADA", LambadaDataset(), [-1], [F1Eval()], [BeamScore(), BALD()]),
        ("TriviaQA", TriviaQADataset(), [-1], [TargetUsageEval()], [BeamScore(), mpnet_cosine(), BLEUVar()]),
        (
            "CommonGen",
            CommonGenDataset(),
            [-1],
    ...
    for task_name, dataset, n_to_validate_list, eval_functions, aq_funcs in tasks:
...
def _evaluate_model_on_benchmark(evaluation_run_config: EvaluationRunConfig) -> None:
    ...
    calc_retention_curve(all_outputs, all_uqs, evaluation_run_config=evaluation_run_config)
