...
from gpt2project.search_methods_gpt import greedy_search_gpt, GPT_search_method, topk_sampling_gpt
...
def evaluate_model_single_example(input_text: str) -> str:
    ...
    output = generate_autoregressivly_gpt2(model, encoding_tensor,search_method=topk_sampling_gpt)
    return output

def generate_autoregressivly_gpt2(model: GPT, tgt_tokens: torch.Tensor, search_method: GPT_search_method, max_tokens: int = 100) -> str:
    model.eval()
