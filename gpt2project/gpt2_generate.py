...
def generate_from_model(
    model: nn.Module, context: str, max_length: int = 100, num_return_sequences: int = 5
) -> None:
    model.eval()
...
def generate_model_single_example(
    model: GPT, tokenizer: tiktoken.Encoding, input_text: str
...
def generate_karpathy(
    model: nn.Module, tgt_tokens: torch.Tensor, max_len: int
) -> AutoregressiveInferenceResults:
    model.eval()
    torch.manual_seed(42)
    torch.cuda.manual_seed(42)
    ...
    return AutoregressiveInferenceResults(
        x, torch.ones(x.size(0), x.size(1)).to(hyperparameters.device)
    )
