...
from uq.generate_with_uq import (
    BatchedValidationResult,
    generate_autoregressivly_with_uq,
)
...
def validate_uq(
    ...
    output_list: List[ValidationResult] = [
        ValidationResult(
            hypothesis=[],
            reference=[],
            uncertainty=torch.tensor([]).to(hyperparameters.device),
        )
        for _ in aq_funcs
    ]
    ...
    with torch.no_grad():
        for i, batch in tqdm(
            ...
            batched_validation_results: List[BatchedValidationResult] = (
                generate_autoregressivly_with_uq(
                    model,
                    src_tokens,
                    ground_truth,
            ...
            for j, aq_func in enumerate(aq_funcs):
                output_list[j].hypothesis.extend(
                    batched_validation_results[j].hypothesis
                )
                output_list[j].reference.extend(batched_validation_results[j].reference)
                output_list[j].uncertainty = torch.cat(
                    [
                        output_list[j].uncertainty,
                        batched_validation_results[j].uncertainty,
                    ],
                    dim=0,
                )
