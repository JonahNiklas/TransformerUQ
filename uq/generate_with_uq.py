...
def generate_autoregressivly_with_uq(
    ...
    softmax_probs : torch.Tensor = torch.zeros(hyperparameters.training.batch_size, hyperparameters.uq.num_inferences, hyperparameters.transformer.max_len, len(vocab)).to(hyperparameters.device)
    for n in tqdm(range(hyperparameters.uq.num_inferences)):
        ...
        softmax_probs[:, n, :, :] = inference_result.softmax_probs
        for i in range(hyperparameters.training.batch_size):
            hypothesis[i].append(output_to_text(inference_result.token_ids[i,n,:].tolist(), lang='en'))
