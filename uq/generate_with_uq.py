...
from data_processing.vocab import BOS_TOKEN, load_vocab, output_to_text
...
def generate_autoregressivly_with_uq(
    ...
    with torch.no_grad():
        ...
        logits = torch.zeros(batch_size, max_len).to(
            device
        )  # probability of the most generated token at each time step
                ...
                for i in range(batch_size):
                    for j in range(1, max_len):
                        if tgt_tokens[i, n, j] == vocab_shared.token_to_id("<eos>"):
                            tgt_tokens[i, n, j + 1 :] = vocab_shared.token_to_id(
                                "<pad>"
                            )
                            break
