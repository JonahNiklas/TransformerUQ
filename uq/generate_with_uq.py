...
def generate_autoregressivly_with_uq(
    ...
    token_ids: torch.Tensor = torch.zeros(
        batch_size,
        hyperparameters.uq.num_inferences,
        hyperparameters.transformer.max_len,
    ).to(hyperparameters.device)
    softmax_probs: torch.Tensor = torch.zeros(
        batch_size,
        hyperparameters.uq.num_inferences,
        hyperparameters.transformer.max_len,
    ).to(hyperparameters.device)
    ...
    for n in range(hyperparameters.uq.num_inferences):
        ...
        for b in range(batch_size):
            hypothesis[b].append(
                output_to_text(inference_result.token_ids[b].tolist(), lang="en")
            )
...
def print_sample_sentences(
    batch_size: int,
    src_tokens: torch.Tensor,
    ground_truth: torch.Tensor,
    text_output: Union[List[str], List[List[str]]],
    uq: torch.Tensor,
